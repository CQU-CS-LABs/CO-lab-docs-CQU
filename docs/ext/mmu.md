...
Virtually Indexed Physically Tagged (VIPT) Cache 是带有MMU的处理器Cache常见的组织形式。

细调过Cache参数的同学们一定注意过，Cache往往出现在影响CPU时序的关键路径上，与此同时若采用PIPT Cache（也就是等待TLB匹配后再把地址送入Cache），其匹配时又串上了一个TLB那么必然对频率造成更大的影响。

如果我们每路Cache Index的用到的虚拟地址部分超过了12位，就需要留意这个设计，因此，我们可以使用VIPT的设计，在TLB匹配之前用虚拟地址作为Cache的Index，送入TLB进行匹配，这样Cache的匹配就可以与TLB的匹配同步进行，避免TLB的存在过多影响频率。

细心的同学一定会想到，若VIPT Cache若采用写回策略，会出现同义问题（synoyms）以及重名（aliasing）问题，即使一个物理地址对应了多个虚拟地址，遇到这部分问题读者可以自己思考结合《超标量处理器设计》P79所提的根据物理地址划分Bank的方法来解决，不过考虑到大多数同学造的Cache每一路最多4KB，因此大家在FPGA上实际造机的时候可能很难遇到。

*如果硬件综合设计期间时间有限，可以让操作系统全部跑在Uncached地址段来简化一些必要的实现。*

### 分支预测问题

在启用MMU的处理器上若使用了分支预测器，可能导致跳转的位置不在TLB中从而导致TLB例外的情况，这样的做法不符合ISA定义甚至有可能导致跳转到了无效指令甚至导致内核无法处理这个Page Fault最终导致进程被杀，因此同学们在设计处理器时需要考虑这种情况，如果一定要达成跑通操作系统的目标，可以尝试简化一些提高性能的设计。

此外，同学们也可以了解一下Meltdown和Spectre漏洞，了解一下MMU权限管理配合Cache和分支预测器可能带来的侧信道问题。
