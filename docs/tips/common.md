...
这里需要注意的是，我们设计的处理器采用的是 **小端序**，也就是高位放在低地址中（卓越班的同学在做计网项目时应该很熟悉这个问题，与网络上的大端序刚好相反），而我们传递的wea的4字节信号则是按照大端序传递的。
...
这里应该会有同学提出疑问，若访问的地址跨越了2个32字节怎么办？这里其实我们在57条指令的时候是需要去做异常处理的，且52条指令中其实不会出现这样的情况，可以当作一个未定义行为随意处理。
...
wire i;
reg a, b;
always_comb begin // always_comb和always @(*)等价
...
经过以上实验可以看出，同学们在流水线每一级内部，若写posedge clk，后续用阻塞赋值，则相当于是使用触发器触发前的结果（比如在E阶段这么写，实际上就是M阶段进行的执行；而在M阶段这么写，实际上也就是WB阶段进行的执行。）。
